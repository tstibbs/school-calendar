...
class DeployStack extends Stack {
	...
	deployBackend() {
		...
		this.#skillLambda = this.#buildLambda(this, 'alexa-skill', Duration.seconds(20), {
			BUCKET_NAME: this.#bucket.bucketName
		})
		...
		this.#bucket.grantReadWrite(docReceiver)
		...
		this.#bucket.addToResourcePolicy(
			new PolicyStatement({
				...
				principals: [docReceiver.grantPrincipal, docExtractor.grantPrincipal, this.#skillLambda.grantPrincipal]
			})
		)
	}
	...
	deployCloudfront() {
		...
		new CfnOutput(this, 'docReceiverEndpoint', {value: `https://${distribution.domainName}`})
	}
}
