...
import {configureErrorNotifyingLambdaHandler} from '@tstibbs/cloud-core-utils/src/stacks/lambda.js'

import {INPUTS, PROMPTS, QUERY_CONFIG} from '../src/config.js'
import {BedrockModelEnabler} from './bedrock-subscriptions.js'
import {
	AMZN_SKILL_ID,
	ALEXA_VENDOR_ID,
	ALEXA_CLIENT_ID,
...
class DeployStack extends Stack {
	...
	subscribeToModels() {
		this.#bedrockEnabler = new BedrockModelEnabler(this, 'ModelAccess', {
			modelIds: [...getExtractionModelIds(), QUERY_CONFIG.modelId]
		})
	}

	createSharedInfra() {
		this.#notificationTopic = new Topic(this, 'notificationTopic')
		this.#notificationTopic.addSubscription(new EmailSubscription(NOTIFICATION_EMAIL))
	}
	...
	deployBackend() {
		...
		docExtractor.addToRolePolicy(
			new PolicyStatement({
				resources: this.#bedrockEnabler.arnsForModelIds(getExtractionModelIds()),
		...
		this.#skillLambda.addToRolePolicy(
			new PolicyStatement({
				resources: [this.#bedrockEnabler.arnForModelId(QUERY_CONFIG.modelId)],
				actions: ['bedrock:InvokeModel']
			})
		)
	...
	#buildLambda(stack, entry, timeout, outputType, envs = {}) {
		...
		configureErrorNotifyingLambdaHandler(handler, this.#notificationTopic)
		return handler
	}
}

function getExtractionModelIds() {
	const promptIds = Object.values(INPUTS)
		.map(input => input?.extraction?.promptId)
		.filter(Boolean)
	const modelIds = Object.entries(PROMPTS)
		.filter(([promptId, prompt]) => promptIds.includes(promptId))
		.map(([promptId, prompt]) => prompt.modelId)
